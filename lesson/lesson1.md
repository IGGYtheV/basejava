...
- Храните все резюме в начале `webapp.storage` (без пустот в виде `null`), чтобы не перебирать каждый раз все 10_000 элементов
...
```
Схема хранения резюме в массиве webapp.storage (в элементах от 0 до size-1 отсутствуют null):
